...
Write-Host "== スタートメニューに登録 ==" -ForegroundColor Cyan
# コントロールパネルの単体起動用ショートカット。ここから『SteamVR を起動』ボタンで
# SteamVR を立ち上げられるので、操作の入口を一元化できる。
try {
    $programs = [Environment]::GetFolderPath("Programs")
    $lnkPath = Join-Path $programs "OpenMeow.lnk"
    $wsh = New-Object -ComObject WScript.Shell
    $shortcut = $wsh.CreateShortcut($lnkPath)
    $shortcut.TargetPath = $overlayExe
    $shortcut.WorkingDirectory = [System.IO.Path]::GetDirectoryName($overlayExe)
    $shortcut.IconLocation = "$overlayExe,0"
    $shortcut.Description = "OpenMeow コントロールパネル(SteamVR の起動もここから)"
    $shortcut.Save()
    Write-Host "スタートメニューに『OpenMeow』を登録しました: $lnkPath"
} catch {
    Write-Host "スタートメニュー登録に失敗しました: $($_.Exception.Message)" -ForegroundColor Yellow
}
